...
class ResultStatistic:
    ...
    def sing_project_results(self, project, type='WPP'):
        ...
        if type == 'WPP':
            dict = {'TN': _result[0], 'TP': _result[1],
                    ...
                    'Classifier': _best2file[3], 'Filter way': _best2file[2]}
        else:
            dict = {'TN': _result[0], 'TP': _result[1],
                    'FN': _result[2],
                    'FP': _result[3], 'pd': _result[4],
    ...
    def statistic(self):
        ...
        columns = ['Source project'] + columns
        df_tpp = DataFrame(columns=columns, index=projects)
        for project in projects:
            _r_wpp = self.sing_project_results(project, 'WPP')
            df_wpp.loc[project] = _r_wpp
            _r_tpp = self.sing_project_results(project, 'TPP')
            df_tpp.loc[project] = _r_tpp

        df_wpp.to_csv(os.path.join(self.root, 'WPP_Best_FARSEC.csv'))
        df_tpp.to_csv(os.path.join(self.root, 'TPP_Best_FARSEC.csv'))
